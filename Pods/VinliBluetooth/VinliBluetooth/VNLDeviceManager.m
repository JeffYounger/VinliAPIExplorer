...
NSString * const VNLDeviceManageriBeaconUUID = @"e2c56db5-dffb-48d2-b060-d0f5a71096e0";
...
@property (strong, nonatomic) NSArray* usersDevices;
...
- (void)setCellularSignalType:(NSString *)cellularSignalType;
- (void)setCellularSignalStrength:(NSNumber *)cellularSignalStrength;
- (void)setHasGPSConnection:(BOOL)hasGPSConnection;
- (void)setHasCellularConnection:(BOOL)hasCellularConnection;
- (void)setHasWifiConnection:(BOOL)hasWifiConnection;
...
- (instancetype)initWithPIDInfo:(NSDictionary *)PIDInfo accessToken:(nonnull NSString *)token
{
    ...
    [self.service getDevicesOnSuccess:^(VLDevicePager *devicePager, NSHTTPURLResponse *response) {
        strongify(self);
        self.usersDevices = devicePager.devices;
    } onFailure:^(NSError *error, NSHTTPURLResponse *response, NSString *bodyString) {
        NSLog(@"Failed to get user devices");
    }];
...
- (void)scanForDevices
{
    ...
    timer = ^{
        ...
        strongify(self)
#if GATT_ENABLED
        [self.bluetoothCentralManager scanForPeripheralsWithServices:@[VNLServiceUUIDMainStream] options:nil];
#else
...
- (BOOL)shouldFinishScanning
{
    ...
    NSLog(@"Scan Identified Devices = %@", identifiedDevices);
#endif
...
//        
        ...
        if (!userDeviceCache)
        {
            ...
            // PUMPKIN - this is where we need to tell my vinli
            NSString *bundleIdentifier = [[NSBundle mainBundle] bundleIdentifier];
            if (![bundleIdentifier isEqualToString:@"li.vin.MyVinli"])
            {
                NSString* urlStr = [NSString stringWithFormat:@"myvinli://?bluetooth=yes&token=%@&deviceChipId=%@&redirectUri=%@", self.accessToken, self.pairedDevice.chipID, [VLSessionManager sharedManager].redirectUri];
...
- (void)disconnectPairedDevice
{
    [self.bluetoothCentralManager cancelPeripheralConnection:self.pairedDevice.peripheral];
}
...
#if GATT_ENABLED
    NSLog(@"Did discover peripheral = %@", [advertisementData objectForKey:CBAdvertisementDataLocalNameKey]);
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
    ...
    if ([characteristic.UUID isEqual:VNLCharacteristicUUIDChipID]) {
        ...
        [self performBlockOnDelegateQueue:^{
            NSError* error;
            NSRegularExpression* regex = [[NSRegularExpression alloc] initWithPattern:@"^[A-Z0-9]{17}$" options:NSRegularExpressionCaseInsensitive error:&error];
            NSString* vinStr = [[NSString alloc] initWithData:characteristic.value encoding:NSASCIIStringEncoding];
            NSInteger matches = [regex numberOfMatchesInString:vinStr options:0 range:NSMakeRange(0, vinStr.length)];
            
            if ([[vinStr substringWithRange:NSMakeRange(0, 4)] isEqualToString:@"NULL"] || matches == 0)
            {
                vinStr = nil;;
            }
            
            device.vehicleIdentificationNumber = vinStr;
    ...
    else if ([characteristic.UUID isEqual:VNLCharacteristicUUIDVehiclePowerStatus])
    {
        ...
        if (!values)
        {
            unichar character = [rawValueStr characterAtIndex:0];
            ...
            else if (character == 'B')
            {
                ...
                {
                    [self performBlockOnDelegateQueue:^{
                        [self.deviceObserver device:device didUpdateBatterVoltage:[NSNumber numberWithFloat:voltage]];
                    }];
                }
            ...
            {
                NSLog(@"Connection = %@", rawValueStr);
                BOOL hasCellConnection = [rawValueStr containsString:@"1"];
                [device setHasCellularConnection:hasCellConnection];
                if (self.deviceObserver && [self.deviceObserver respondsToSelector:@selector(device:didUpdateCellularConnectionState:)])
                {
                    [self performBlockOnDelegateQueue:^{
                        [self.deviceObserver device:device didUpdateCellularConnectionState:hasCellConnection];
                    }];
            ...
            {
                NSLog(@"GPS = %@", rawValueStr);
                
                BOOL hasGPS = [rawValueStr containsString:@"1"];
                [device setHasGPSConnection:hasGPS];
                
                if (self.deviceObserver && [self.deviceObserver respondsToSelector:@selector(device:didUpdateGPSState:)])
                {
                    [self performBlockOnDelegateQueue:^{
                        [self.deviceObserver device:device didUpdateGPSState:hasGPS];
                    }];
                    
                }
                return;
            }
            else if ([rawValueStr containsString:@"S:"])
            {
                NSLog(@"Signal = %@", rawValueStr);
                
                NSString* values = [rawValueStr substringFromIndex:2];
                NSRange commaRange = [values rangeOfString:@","];
                
                if (commaRange.location == NSNotFound) {
                    return;
                }
                
                NSString* signalType = [values substringToIndex:commaRange.location];
                
                if (commaRange.location + 1 > values.length) {
                    return;
                }
                NSString* decibalStr = [values substringFromIndex:commaRange.location + 1];
                NSInteger decibals = [decibalStr integerValue];
                
                [device setCellularSignalType:decibalStr];
                [device setCellularSignalStrength:@(decibals)];
                
                if (self.deviceObserver && [self.deviceObserver respondsToSelector:@selector(device:didUpdateCellularSignalType:decibals:)])
                {
                    [self performBlockOnDelegateQueue:^{
                        [self.deviceObserver device:device didUpdateCellularSignalType:signalType decibals:@(decibals)];
                    }];
                    
                }
                return;
            }
            else if ([rawValueStr containsString:@"SVER:"])
            {
                NSLog(@"BLE Firmware Version = %@", rawValueStr);
                return;
            }
...
- (void)scanForBeacons
{
    NSUUID* VinliUUID = [[NSUUID alloc] initWithUUIDString:VNLDeviceManageriBeaconUUID];
...
- (void)locationManager:(CLLocationManager *)manager didEnterRegion:(CLRegion *)region
{
    ...
    if (self.deviceObserver && [self.deviceObserver respondsToSelector:@selector(deviceManager:deviceDidEnterRegion:)])
    {
        [self performBlockOnDelegateQueue:^{
            [self.deviceObserver deviceManager:self deviceDidEnterRegion:nil];
        }];
    }
...
- (void)locationManager:(CLLocationManager *)manager didExitRegion:(CLRegion *)region
{
    ...
    if (self.deviceObserver && [self.deviceObserver respondsToSelector:@selector(deviceManager:deviceDidExitRegion:)])
    {
        [self performBlockOnDelegateQueue:^{
            [self.deviceObserver deviceManager:self deviceDidExitRegion:nil];
        }];
    }
}
...
- (void)locationManager:(CLLocationManager *)manager didRangeBeacons:(NSArray *)beacons inRegion:(CLBeaconRegion *)region
{
    ...
    for (CLBeacon* beacon in beacons)
    {
        ...
        if (userDeviceCache.allValues.count > 0)
        {
            [userDeviceCache enumerateKeysAndObjectsUsingBlock:^(NSString* key, VNLDevice* device, BOOL *stop) {
                if ([last4DigitsofChipId isEqualToString:[key substringWithRange:NSMakeRange(key.length - 4, 4)]])
                {
                    ...
                    {
                        [self performBlockOnDelegateQueue:^{
                            [self.deviceObserver deviceManager:self deviceDidEnterRegion:device];
                        }];
                    }
                    foundDevice = YES;
                    *stop = YES;
        ...
        if (!foundDevice)
        {
            [self.usersDevices enumerateObjectsUsingBlock:^(VLDevice* device, NSUInteger idx, BOOL * _Nonnull stop) {
                if ([last4DigitsofChipId isEqualToString:[device.chipID substringWithRange:NSMakeRange(device.chipID.length - 4, 4)]])
                {
                    NSLog(@"Beacon matches a users device");
                    if (self.deviceObserver && [self.deviceObserver respondsToSelector:@selector(deviceManager:deviceDidEnterRegion:)])
                    {
                        VNLDevice* vnlDevice;
                        for (VNLDevice* vinliDevice in self.vinliDevices.allValues)
                        {
                            if ([vinliDevice.chipID isEqualToString:device.chipID])
                            {
                                vnlDevice = vinliDevice;
                                break;
                            }
                        }
                        
                        if (!vnlDevice) {
                            NSLog(@"Unable to match user device with connected vinli device from iBeacon");
                            [self scanForDevices];
                            return;
                        }
                        
                        [self performBlockOnDelegateQueue:^{
                            [self.deviceObserver deviceManager:self deviceDidEnterRegion:vnlDevice];
                        }];
                        
                        [self pairWithDevice:vnlDevice];
                    }
